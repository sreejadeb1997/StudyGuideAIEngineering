...
- **Two levels of decomposition** — The system understands a question at two very different altitudes. The orchestrator decides *which domains of data* the question touches, while each skill separately breaks the same question into fine-grained sub-questions that only make sense inside its own domain. Neither level knows how the other one decomposed the work, which is a deliberate design choice, not an accident.

- **Routing that also judges dependency in one step** — When a question arrives, the orchestrator does two things in a single reasoning pass: it picks the relevant specialist skills, and it decides whether those skills can run side by side or whether one actually depends on another's output. That dependency judgment is built directly into the routing decision instead of being worked out later by a separate planning stage.

- **Entity resolved once, then broadcast** — Before anything is handed off, the orchestrator turns any real-world entity in the question (say, a company name) into a single canonical identifier, and it does this exactly once. That resolved identity is passed down to every skill unchanged, so no skill ever has to guess at or re-resolve who the user meant.

- **Clean packages instead of shared memory** — The orchestrator and the skills never share state or talk to each other directly. The orchestrator sends each skill a clean, structured request package and receives a clean, structured result package back. There is no free-form chatter and no shared scratchpad.

- **In-skill filtering before any work is done** — Each skill doesn't blindly answer every sub-question it can imagine. It keeps only the ones that match its own domain knowledge with high confidence and discards anything ambiguous or anything that really belongs to a neighboring skill — and it does this *before* building or running any query, so effort is never wasted on branches that were never its job.

- **Answer delivery separated from quality judgment** — The decision about whether an answer "passed" or "failed" is deliberately decoupled from delivering it. The user gets their response immediately, while the more careful quality check happens afterward in the background, where it is recorded without ever slowing down or altering what the user already received.
...
- **A fixed, one-directional flow** — Everything a skill needs moves through the orchestrator in a fixed shape: resolved entity information travels *down* to the skills in one identical request package, and retrieved facts travel back *up* in a uniform result package.

- **Each skill knows what isn't its job** — Every skill carries its own machine-readable description of the sub-intents that *look* like they belong to it but actually belong to a neighboring skill. When it breaks a question down, it checks its candidates against that "not mine" list and quietly drops the ones that aren't its responsibility.

- **De-duplication decided locally, not negotiated** — Because each skill resolves ownership on its own against that list, overlapping requests get de-duplicated in a distributed way with no back-and-forth. The end result is that any given sub-intent is answered by exactly one skill, decided cleanly at the boundary.

---

## Subquestion — How are recursive sub-intent graphs generated and expanded?

- **The shape of the work is decided at routing time** — Whether a question is a set of independent tasks or a chain of dependent ones is decided the moment it is routed, not discovered later by a planner. The router raises one of two signals: a set of independent domains that can run in parallel, or a single marker meaning "this is nested — one part feeds another."

- **Independent questions simply fan out** — When the parts don't depend on each other, each domain is asked its piece in parallel and the pieces are recombined afterward.

- **Dependent questions are compiled into one query** — Instead of running the first part, capturing its results, feeding them back through the model, and then running the second part, the system compiles the entire dependency into a single query: the part that produces the intermediate result becomes an inner stage, and the part that consumes it is expressed as a join against that stage. The whole dependency is resolved atomically, inside the data engine, in one round-trip.

---

## Subquestion — How are confidence scores calculated, branches pruned, and fallbacks triggered?

- **Confidence judged where the knowledge lives** — Rather than a single central scorer that has to know everything, each skill judges confidence for itself, comparing its candidate sub-questions against its own domain description and worked examples and only admitting the ones that match cleanly and unambiguously.

- **Pruning happens before any cost is incurred** — Anything low-confidence, ambiguous, or belonging to a neighboring skill is dropped *before* a query is generated or run, so the pruning saves both a model call and a data lookup.

- **A deterministic fallback at every stage** — The system is built to keep working when pieces stumble: if the question can't be cleanly reformulated it proceeds with the original wording; if it can't be split sensibly it's treated as one whole question; if one branch fails the others carry on; and if a provider is overloaded the request is quietly re-routed. No single hiccup takes down the whole request.

- **Quality judgment kept off the critical path** — Clear, terminal failures fall back to a defined safe response immediately, while the nuanced "did this actually answer the question" check runs out of band. Its verdict is recorded for later but never delays or rewrites the answer the user already has.

---

## Subquestion — Which portions of the workflow differ from existing planner-based multi-agent systems?

- **Two-level decomposition** — It decomposes intent at two altitudes (by domain at the top, by sub-question inside each skill), rather than once into a single flat task list the way a conventional planner does.

- **Dependencies compiled, not sequenced** — It turns a dependency between tasks into one atomic query, rather than a sequence of tool calls whose intermediate results must be passed from step to step.

- **Topology chosen at routing time** — It commits to the *shape* of the work in the same step it decides *what* the work is: one routing inference emits both the relevant domains and how they relate — a flat parallel set, or a distinct "nested" signal when one part feeds another. The execution path is fixed before any tool runs. A conventional planner instead builds a task graph that an agent loop must hold, interpret, re-evaluate after each step, and walk node by node. Here there is no such loop and no evolving graph — the branch is picked once and executed, which makes the flow deterministic and inspectable, removes per-step re-planning latency, and avoids failure modes like stalled loops and mis-scheduled dependencies.

- **Identity resolved once** — It resolves entity identity a single time and broadcasts it, rather than resolving it again inside each tool.

- **Distributed de-duplication** — It avoids overlapping work through each skill's own exclusion knowledge, rather than relying on a planner's reasoning or a shared memory. This also keeps token usage low: no single model call ever has to ingest every domain's full metadata at once to work out who owns what. Instead, each skill carries only its own limited slice of metadata (its domain description plus its "not mine" exclusion list) and reasons over just that slice, and the skills do this in parallel. Splitting a large context into small per-skill contexts that run concurrently means far fewer tokens per call, which sidesteps the token-limit ceiling and the rate/throughput throttling that a single monolithic, all-metadata prompt would run into.

- **Security woven into the query** — It builds access control into every stage of the compiled query, rather than applying it separately at each individual tool call.
...
```mermaid
flowchart TD
    U([User query:\ntop 5 opportunities + contacts for those]) --> RT{Router:\ndependency detected?}
    RT -->|Yes: producer -> consumer| NEST[Route as single NESTED request]
    NEST --> COMP[Compile dependency into ONE query\nproducer = inner stage\nconsumer = join on inner stage\naccess-control woven into every stage]
    COMP --> EXEC[Execute atomically\nsingle round-trip in data engine]
    EXEC --> GND[Ground facts into answer\n+ follow-up prompts]
    GND --> DEL([Deliver to user immediately])
    DEL -.-> ADJ[[Background: quality adjudication\n& observability - off critical path]]

    classDef async fill:#eee,stroke:#999,stroke-dasharray:4 3;
    class ADJ async;
```

- **Recognize the dependency at routing** — It sees that the contacts are wanted *for those* opportunities and, because one part depends on another, routes the whole thing as a single nested request rather than two independent lookups. 

- **Compile the dependency into one query** — The "top opportunities" part becomes an inner stage and the "contacts for those opportunities" part is expressed as a join against that stage, with the required access-control rules built into every stage.
...
```mermaid
flowchart TD
    U([User query:\nconsumption trend + risk signals + key contacts for Contoso]) --> CU[Clean up & identify account]
    CU --> RES[Resolve account once\nContoso -> canonical TPID]
    RES --> RT{Router:\ndependency detected?}
    RT -->|No: independent| FAN[Fan out to independent skills\nFIRST-LEVEL decomposition by domain]
    ...
    SECOND --> FLT[Per-skill confidence filter\ndrop ambiguous / not-mine sub-questions]
    FLT --> GEN[Generate + execute one query per sub-question\nin parallel, access-control on each]
    GEN --> MRG[Merge facts -> ground into one answer\n+ follow-up prompts]
...
- **Route into multiple independent skills (first level of decomposition)** — The router recognizes there is no dependency between the parts and emits a flat set of domains — for example a **Consumption** skill, an **External Signals** skill, and a **Contacts** skill in this question's case— to run in parallel rather than a single nested route.

- **Each skill decomposes into its own sub-questions (second level of decomposition)** — Working only against its own limited metadata and in parallel with the others:
...
- **Filter each sub-question before doing work** — Every skill keeps only the sub-questions that match its own domain with high confidence and drops anything ambiguous or owned by a sibling (via its "not mine" exclusion list), so no two skills answer the same slice and no wasted queries are generated.
...
- **How this differs** — All of the above run the task by *walking a mutable graph or conversation loop*, applying security at each node/tool call and often checking quality in-line. This approach differs that model on five specific points:
  ...
  3. **Compile dependencies, don't sequence them** — a cross-domain dependency becomes one atomic, uniformly-governed query executed in a single round-trip, rather than a chain of tool calls passing results between them.
  4. **Bounded per-skill reasoning, not one giant prompt** — confidence-scoring and pruning happen inside each domain's small metadata slice in parallel, avoiding the monolithic, throttling-prone reasoning pass that a single all-tools prompt incurs.
...
- **How this differs** — Skills here never talk to each other, never share mutable state, and never negotiate ownership. Coordination is strictly one-directional through the orchestrator (canonical entities down, uniform facts up), and cross-skill overlap is resolved *locally and declaratively* by each skill's own "not-mine" exclusion metadata — eliminating inter-agent chatter and the token cost of a shared, all-context reasoning step.
...
- **Seller and account-research Copilots** — Directly targets Microsoft's sales/account-intelligence experiences (e.g. MSX / Copilot for Sales-style scenarios) where a seller asks compound, cross-domain questions about an account and expects one grounded answer.
...
- **Governed enterprise-data NL interfaces** — Any product that puts natural language over Fabric / Azure SQL / semantic models with row-level security benefits from the compile-dependency-into-one-governed-query approach.

## Subquestion — Expected competitive differentiation

- **Atomic, governed multi-hop answers** — Answering dependent multi-domain questions in a single uniformly-secured query is a latency and correctness advantage over competitors that chain tool calls and re-apply security per hop.
- **Scales metadata without token blow-up** — Per-skill bounded metadata reasoning in parallel avoids the token-limit/throttling ceiling that monolithic all-tool prompts hit, so new domains can be onboarded without degrading routing.
- **Deterministic, inspectable control flow** — Topology chosen at routing time yields predictable, auditable execution (important for enterprise trust/compliance) versus opaque agent loops.
- **Security-first by construction** — Row-level access woven into every stage of the compiled query is a strong enterprise differentiator over tool-call-site security.
...
- **Onboarding model for new skills** — New domains plug in by shipping only their own metadata + exclusion set, making it a scalable pattern for a growing skill marketplace/ecosystem.
...
- **Status / date** — 1st June deployed in production. Integrated into Sales Agent.

## Subquestion — Customer preview

- **Status / date** — 1st June (for consumption by sellers- ATUs, CSUs, STUs).

## Subquestion — Public demonstration

- **Status / date** — Not yet done
